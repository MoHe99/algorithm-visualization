...
const bubbleSortInformation: Information = {
  name: 'Bubble Sort',
  description: <div>Conceptually, a merge sort works as follows: <ol><li>Divide the unsorted list into n sublists, each containing one element (a list of one element is considered sorted).</li><li>Repeatedly merge sublists to produce new sorted sublists until there is only one sublist remaining. This will be the sorted list.</li></ol></div>,
  runtime: 'O(n/log n)',
...
const SortingVisualizer = (): ReactElement => {
  ...
  const [information, setInformation] = useState<Information>(bubbleSortInformation);
...
/**
  ...
  * */
  ...
  const merge =  async (array:number[], low:number, mid:number, high:number): Promise<void> => {
    let i = low
    let j = mid + 1
    let k = 0
    let mergedList = []
  ...
  return (
    <div className='field'>
      <div className='header'>
        ...
        <div className='menu'>
          <select disabled={buttonDisable} onChange={(e) => {
            resetArray();
          ...
          <button className='button-sort' disabled={buttonDisable} onClick={handleSorting}>Sort</button>
          <button className='button-reset' disabled={buttonDisable} onClick={resetArray}>Shuffle</button>
          <button className='button-exit' disabled={!buttonDisable} onClick={refreshPage}>Terminate</button>
        </div>
      </div>
      <div className='main-content'>
        <div className="sort-bars">
          {sortingArray.slice(0, range).map((value, index) => (
            <div className='bar' id={index.toString()} key={index} style={{height: `${value}px`}}></div>
          ))}
        </div>
        ...
        <div className='algorithm-information'>
          ...
          <div className='stats'>
            <div className='row'><div className='cell description'>Worst-case runtime</div><div className='cell'>{information?.runtime}</div></div>
            <div className='row'><div className='cell description'>Worst-case space-complexety</div><div className='cell'>{information?.space_complexety}</div></div>
          </div>
        </div>
      </div>
    </div>
